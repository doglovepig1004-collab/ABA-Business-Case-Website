...
<head>
    ...
    <title>Banking Positions - Banking on the Future</title>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark">
        <div class="container">
            ...
            <div class="nav-actions">
                ...
                <a class="nav-button nav-home" href="/positions">Positions</a>
    ...
    <div class="container mt-4 flex-grow-1">
        <div class="hero text-center">
            <h1 class="display-3 hero-title">Explore Banking Positions</h1>
            <p class="lead">See the many teams, specialties, and entry points that power modern banking.</p>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Explore banking careers, find your role match, prepare your resume, and connect with ABA's talent network.">
    ...
    <style>
        :root {
            ...
            --aba-gold: #dc9f1d;
            --aba-slate: #4e7c9c;
            --aba-sans: "Helvetica Neue", Helvetica, Arial, sans-serif;
            --aba-serif: "Minion Pro", "Times New Roman", Times, serif;
            --ink: #071a28;
            --deep: #03111b;
            --panel: rgba(9, 39, 58, 0.72);
            --line: rgba(132, 197, 229, 0.19);
        }
        * { box-sizing: border-box; }
        ...
        body {
            ...
            overflow-x: hidden;
            background:
                radial-gradient(circle at 82% 7%, rgba(78,124,156,0.3), transparent 28rem),
                radial-gradient(circle at 10% 40%, rgba(0,90,140,0.22), transparent 30rem),
                var(--deep);
            color: #edf7fb;
            font-family: var(--aba-sans);
        }
        body::before {
            content: '';
            position: fixed;
            ...
            z-index: -1;
            pointer-events: none;
            background-image: linear-gradient(rgba(119,190,224,0.045) 1px, transparent 1px), linear-gradient(90deg, rgba(119,190,224,0.045) 1px, transparent 1px);
            background-size: 54px 54px;
            background-position-y: var(--grid-offset, 0px);
            mask-image: linear-gradient(to bottom, black, transparent 82%);
        }
        a { color: inherit; }
        .scroll-progress { position: fixed; inset: 0 0 auto; z-index: 40; height: 3px; background: linear-gradient(90deg, var(--aba-gold), #f2cc6c, #7cc6e8); transform: scaleX(0); transform-origin: left; box-shadow: 0 0 12px rgba(220,159,29,0.55); }
        .navbar { position: sticky; top: 0; z-index: 20; padding: 0.9rem 0; background: rgba(0,90,140,0.9) !important; border-bottom: 1px solid rgba(255,255,255,0.14); backdrop-filter: blur(18px); }
        .navbar-brand { color: #fff !important; font-family: 'Cormorant Garamond', Georgia, serif; font-size: 1.65rem; font-weight: 700; letter-spacing: 0.015em; line-height: 1; display: inline-flex; align-items: center; gap: 11px; }
        .brand-logo { display: block; width: 42px; height: 42px; flex: 0 0 auto; border-radius: 10px; box-shadow: 0 2px 9px rgba(0,0,0,0.18); }
        .brand-copy { display: flex; flex-direction: column; gap: 4px; }
        .brand-title { line-height: 0.9; }
        .brand-highlight { color: #f0c45b; font-weight: 600; }
        .brand-attribution { color: rgba(255,255,255,0.72); font-family: var(--aba-sans); font-size: 0.56rem; font-weight: 700; letter-spacing: 0.12em; line-height: 1; text-transform: uppercase; }
        .nav-actions { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 0.55rem; }
        .nav-button { padding: 0.5rem 0.88rem; border: 1px solid rgba(255,255,255,0.55); border-radius: 999px; color: #fff; font-size: 0.87rem; font-weight: 700; text-decoration: none; transition: transform 0.2s, background 0.2s, border-color 0.2s; }
        .nav-button:hover { color: var(--aba-blue); background: #fff; transform: translateY(-1px); }
        .nav-primary { border-color: var(--aba-gold); background: var(--aba-gold); color: #fff; }
        .nav-primary:hover { border-color: #bd8413; background: #bd8413; color: #fff; }
        .nav-button:focus-visible, .action:focus-visible, .journey-card:focus-visible { outline: 3px solid rgba(220,159,29,0.55); outline-offset: 3px; }
        .hero { position: relative; min-height: 720px; display: grid; align-items: center; padding: clamp(5rem, 9vw, 8rem) 0 6rem; overflow: hidden; }
        .hero::before { content: ''; position: absolute; right: -13rem; top: 2rem; width: 38rem; height: 38rem; border: 1px solid rgba(106,184,221,0.18); border-radius: 50%; box-shadow: 0 0 0 4rem rgba(0,90,140,0.035), 0 0 0 8rem rgba(0,90,140,0.025); }
        .hero-layout { position: relative; z-index: 1; display: grid; grid-template-columns: minmax(0, 1.12fr) minmax(320px, 0.88fr); gap: clamp(2.5rem, 7vw, 6rem); align-items: center; }
        .eyebrow { display: inline-flex; align-items: center; gap: 0.6rem; margin-bottom: 1.4rem; color: #f1c65f; font-size: 0.78rem; font-weight: 800; letter-spacing: 0.15em; text-transform: uppercase; }
        .eyebrow::before { content: ''; width: 2.3rem; height: 2px; background: var(--aba-gold); box-shadow: 0 0 12px rgba(220,159,29,0.65); }
        .hero h1 { max-width: 780px; margin: 0; font-size: clamp(3.1rem, 7vw, 6.5rem); font-weight: 700; letter-spacing: -0.058em; line-height: 0.94; }
        .hero h1 span { display: block; color: #79c1e3; }
        .hero-lead { max-width: 660px; margin: 1.75rem 0 0; color: rgba(235,247,252,0.78); font-family: var(--aba-serif); font-size: clamp(1.08rem, 2vw, 1.28rem); line-height: 1.65; }
        .hero-actions { display: flex; flex-wrap: wrap; gap: 0.85rem; margin-top: 2rem; }
        .action { display: inline-flex; align-items: center; justify-content: center; gap: 0.65rem; min-height: 50px; padding: 0.8rem 1.3rem; border: 1px solid rgba(255,255,255,0.28); border-radius: 999px; color: #fff; font-weight: 800; text-decoration: none; transition: transform 0.2s, box-shadow 0.2s, background 0.2s; }
        .action:hover { color: #fff; transform: translateY(-2px); box-shadow: 0 12px 30px rgba(0,0,0,0.2); }
        .action-primary { border-color: var(--aba-gold); background: var(--aba-gold); }
        .action-primary:hover { background: #bd8413; }
        .action-secondary { background: rgba(255,255,255,0.07); backdrop-filter: blur(8px); }
        .action-secondary:hover { background: rgba(255,255,255,0.14); }
        .proof-strip { display: flex; flex-wrap: wrap; gap: 0.7rem 1.2rem; margin-top: 2rem; color: rgba(229,244,251,0.64); font-size: 0.82rem; font-weight: 700; letter-spacing: 0.03em; }
        .proof-strip span { display: inline-flex; align-items: center; gap: 0.45rem; }
        .proof-strip span::before { content: ''; width: 6px; height: 6px; border-radius: 50%; background: var(--aba-gold); box-shadow: 0 0 10px rgba(220,159,29,0.75); }
        .signal-panel { position: relative; min-height: 510px; padding: 1.5rem; overflow: hidden; border: 1px solid rgba(133,205,238,0.24); border-radius: 30px; background: linear-gradient(155deg, rgba(15,62,88,0.92), rgba(4,25,38,0.94)); box-shadow: 0 34px 90px rgba(0,0,0,0.34), inset 0 1px rgba(255,255,255,0.1); backdrop-filter: blur(18px); }
        .signal-panel::before { content: ''; position: absolute; right: -7rem; top: -9rem; width: 22rem; height: 22rem; border-radius: 50%; background: radial-gradient(circle, rgba(53,161,209,0.28), transparent 68%); }
        .signal-panel::after { content: ''; position: absolute; inset: 0; pointer-events: none; background-image: linear-gradient(rgba(126,198,231,0.045) 1px, transparent 1px), linear-gradient(90deg, rgba(126,198,231,0.045) 1px, transparent 1px); background-size: 30px 30px; mask-image: linear-gradient(to bottom right, black, transparent 72%); }
        .signal-frame { position: relative; z-index: 1; min-height: 460px; display: flex; flex-direction: column; padding: 1.15rem; border: 1px solid rgba(144,209,239,0.14); border-radius: 20px; background: rgba(2,20,31,0.3); }
        .signal-header { display: flex; justify-content: space-between; gap: 1rem; align-items: flex-start; padding-bottom: 1.05rem; border-bottom: 1px solid rgba(255,255,255,0.1); }
        .signal-kicker { display: block; color: #82c8e8; font-size: 0.68rem; font-weight: 800; letter-spacing: 0.15em; text-transform: uppercase; }
        .signal-heading { display: block; margin-top: 0.28rem; color: #fff; font-size: 1rem; font-weight: 800; }
        .signal-live { display: inline-flex; align-items: center; gap: 0.42rem; padding: 0.42rem 0.65rem; border: 1px solid rgba(220,159,29,0.3); border-radius: 999px; background: rgba(220,159,29,0.08); color: #f0c45b; font-size: 0.63rem; font-weight: 800; letter-spacing: 0.09em; text-transform: uppercase; }
        .signal-live::before { content: ''; width: 7px; height: 7px; border-radius: 50%; background: var(--aba-gold); box-shadow: 0 0 10px rgba(220,159,29,0.9); animation: signalPulse 2.2s ease-in-out infinite; }
        .signal-stat { display: grid; grid-template-columns: auto 1fr; gap: 1.15rem; align-items: end; padding: 1.35rem 0 1.2rem; }
        .signal-count { color: #fff; font-size: clamp(4.2rem, 8vw, 6.3rem); font-weight: 700; letter-spacing: -0.075em; line-height: 0.82; }
        .signal-stat p { max-width: 150px; margin: 0; padding-bottom: 0.15rem; color: rgba(235,247,252,0.62); font-size: 0.8rem; font-weight: 600; line-height: 1.4; }
        .signal-spectrum { display: grid; gap: 0.78rem; padding: 1.15rem 0; border-top: 1px solid rgba(255,255,255,0.08); border-bottom: 1px solid rgba(255,255,255,0.08); }
        .signal-row { display: grid; grid-template-columns: minmax(112px, 0.9fr) minmax(100px, 1.25fr) 1.8rem; gap: 0.7rem; align-items: center; }
        .signal-row > span { color: rgba(235,247,252,0.76); font-size: 0.72rem; font-weight: 700; }
        .signal-row > b { color: rgba(130,200,232,0.65); font-size: 0.62rem; letter-spacing: 0.08em; text-align: right; }
        .signal-track { height: 5px; overflow: hidden; border-radius: 999px; background: rgba(255,255,255,0.08); }
        .signal-track i { display: block; width: var(--fill); height: 100%; border-radius: inherit; background: linear-gradient(90deg, var(--aba-blue), #7bc7e9); box-shadow: 0 0 12px rgba(83,179,222,0.42); transform-origin: left; }
        .signal-row:nth-child(3) .signal-track i { background: linear-gradient(90deg, #9b7118, var(--aba-gold)); }
        .has-motion .signal-track i { transform: scaleX(0); transition: transform 0.9s cubic-bezier(0.22,1,0.36,1) 0.35s; }
        .has-motion .signal-panel.is-visible .signal-track i { transform: scaleX(1); }
        .signal-footer { display: grid; grid-template-columns: repeat(3, minmax(0,1fr)); gap: 0.55rem; margin-top: auto; padding-top: 1.05rem; }
        .signal-metric { min-width: 0; padding: 0.75rem; border: 1px solid rgba(255,255,255,0.08); border-radius: 12px; background: rgba(255,255,255,0.035); }
        .signal-metric strong { display: block; color: #fff; font-size: 0.82rem; }
        .signal-metric span { display: block; margin-top: 0.22rem; color: rgba(235,247,252,0.5); font-size: 0.62rem; line-height: 1.25; }
        @keyframes signalPulse { 50% { opacity: 0.5; transform: scale(0.78); } }
        .has-motion .reveal { opacity: 0; filter: blur(5px); transform: translateY(34px) scale(0.988); transition: opacity 0.72s ease, transform 0.72s cubic-bezier(0.22,1,0.36,1), filter 0.72s ease; }
        .has-motion .reveal-left { transform: translateX(-38px); }
        .has-motion .reveal-right { transform: translateX(38px); }
        .has-motion .reveal.is-visible { opacity: 1; filter: blur(0); transform: translate(0) scale(1); }
        .reveal-delay-1 { transition-delay: 70ms !important; }
        .reveal-delay-2 { transition-delay: 140ms !important; }
        .reveal-delay-3 { transition-delay: 210ms !important; }
        .section { position: relative; padding: clamp(5rem, 9vw, 8rem) 0; }
        .section-light { background: #f5f8fa; color: var(--ink); }
        .section-light::before { content: ''; position: absolute; inset: 0; background: linear-gradient(130deg, rgba(0,90,140,0.05), transparent 40%); pointer-events: none; }
        .section-heading { max-width: 790px; margin-bottom: 3rem; }
        .section-label { color: var(--aba-blue); font-size: 0.76rem; font-weight: 800; letter-spacing: 0.15em; text-transform: uppercase; }
        .section-dark .section-label { color: #79c1e3; }
        .section-heading h2 { margin: 0.65rem 0 0; font-size: clamp(2.2rem, 5vw, 4.2rem); font-weight: 700; letter-spacing: -0.045em; line-height: 1.04; }
        .section-heading p { margin: 1rem 0 0; color: #60717e; font-family: var(--aba-serif); font-size: 1.12rem; line-height: 1.65; }
        .section-dark .section-heading p { color: rgba(235,247,252,0.68); }
        .insight-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 1rem; }
        .insight-card { position: relative; min-height: 265px; padding: 1.55rem; overflow: hidden; border: 1px solid rgba(0,90,140,0.13); border-radius: 22px; background: rgba(255,255,255,0.82); box-shadow: 0 14px 35px rgba(15,45,63,0.07); }
        .insight-card::after { content: attr(data-index); position: absolute; right: 1rem; bottom: -1.1rem; color: rgba(0,90,140,0.06); font-size: 6rem; font-weight: 800; line-height: 1; }
        .insight-icon { display: grid; place-items: center; width: 46px; height: 46px; border-radius: 14px; background: #e2f0f7; color: var(--aba-blue); font-weight: 800; }
        .insight-card h3 { position: relative; z-index: 1; margin: 1.35rem 0 0.65rem; color: var(--aba-blue); font-size: 1.15rem; font-weight: 800; }
        .insight-card p { position: relative; z-index: 1; margin: 0; color: #5e6a72; font-family: var(--aba-serif); line-height: 1.58; }
        .journey-grid { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 1rem; counter-reset: journey; }
        .journey-card { position: relative; min-height: 310px; padding: 1.55rem; border: 1px solid var(--line); border-radius: 22px; background: var(--panel); color: #fff; text-decoration: none; transition: transform 0.25s, border-color 0.25s, background 0.25s; counter-increment: journey; }
        .journey-card:hover { color: #fff; transform: translateY(-6px); border-color: rgba(220,159,29,0.55); background: rgba(12,60,87,0.82); }
        .journey-card::before { content: '0' counter(journey); display: block; margin-bottom: 3.8rem; color: var(--aba-gold); font-size: 0.82rem; font-weight: 800; letter-spacing: 0.1em; }
        .journey-card::after { content: ''; position: absolute; left: calc(100% + 1px); top: 2.15rem; width: 1rem; height: 1px; background: var(--aba-gold); opacity: 0.55; }
        .journey-card:last-child::after { display: none; }
        .journey-card h3 { margin: 0 0 0.65rem; font-size: 1.25rem; font-weight: 800; }
        .journey-card p { margin: 0; color: rgba(235,247,252,0.66); font-family: var(--aba-serif); line-height: 1.55; }
        .journey-link { position: absolute; left: 1.55rem; bottom: 1.4rem; color: #82c9e9; font-size: 0.82rem; font-weight: 800; }
        .perspective { display: grid; grid-template-columns: 0.9fr 1.1fr; gap: clamp(2rem, 6vw, 6rem); align-items: center; }
        .perspective-mark { position: relative; min-height: 430px; border-radius: 28px; background: linear-gradient(145deg, var(--aba-blue), var(--aba-slate)); overflow: hidden; box-shadow: 0 25px 60px rgba(0,90,140,0.2); }
        .perspective-mark::before { content: ''; position: absolute; width: 420px; height: 420px; left: -150px; top: -150px; border: 1px solid rgba(255,255,255,0.2); border-radius: 50%; box-shadow: 0 0 0 55px rgba(255,255,255,0.035), 0 0 0 110px rgba(255,255,255,0.025); }
        .perspective-copy { position: absolute; inset: auto 2rem 2rem; max-width: 390px; }
        .perspective-copy strong { display: block; color: #f2c75d; font-size: 0.77rem; letter-spacing: 0.12em; text-transform: uppercase; }
        .perspective-copy span { display: block; margin-top: 0.7rem; color: #fff; font-size: clamp(1.65rem, 4vw, 2.7rem); font-weight: 700; letter-spacing: -0.04em; line-height: 1.08; }
        .perspective-text h2 { margin: 0; color: var(--aba-blue); font-size: clamp(2.2rem, 5vw, 4rem); font-weight: 700; letter-spacing: -0.045em; line-height: 1.04; }
        .perspective-text > p { margin: 1.2rem 0 0; color: #5f6d76; font-family: var(--aba-serif); font-size: 1.12rem; line-height: 1.7; }
        .transfer-list { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 0.7rem; margin-top: 1.6rem; }
        .transfer-list span { padding: 0.78rem 0.9rem; border-left: 3px solid var(--aba-gold); background: #fff; color: #354956; font-size: 0.87rem; font-weight: 700; box-shadow: 0 5px 18px rgba(17,51,69,0.05); }
        .decision-section { overflow: hidden; background: #eaf1f4; color: var(--ink); }
        .decision-section::before { content: ''; position: absolute; right: -12rem; top: -13rem; width: 34rem; height: 34rem; border: 1px solid rgba(0,90,140,0.1); border-radius: 50%; box-shadow: 0 0 0 5rem rgba(0,90,140,0.025), 0 0 0 10rem rgba(0,90,140,0.018); }
        .decision-layout { position: relative; display: grid; grid-template-columns: minmax(0,0.9fr) minmax(0,1.1fr); gap: clamp(2.5rem, 7vw, 6rem); align-items: start; }
        .decision-intro { position: sticky; top: 8rem; }
        .decision-intro h2 { margin: 0.7rem 0 0; color: var(--aba-blue); font-size: clamp(2.25rem, 5vw, 4.25rem); font-weight: 700; letter-spacing: -0.05em; line-height: 1.02; }
        .decision-intro > p { margin: 1.2rem 0 0; color: #5a6972; font-family: var(--aba-serif); font-size: 1.12rem; line-height: 1.68; }
        .role-signal { margin-top: 1.7rem; padding: 1.2rem; border: 1px solid rgba(0,90,140,0.14); border-radius: 18px; background: rgba(255,255,255,0.62); box-shadow: 0 12px 30px rgba(17,57,77,0.06); }
        .role-signal span { display: block; color: var(--aba-gray); font-size: 0.68rem; font-weight: 800; letter-spacing: 0.13em; text-transform: uppercase; }
        .role-signal strong { display: block; margin-top: 0.45rem; color: #17374a; font-size: 1.03rem; line-height: 1.45; }
        .decision-lenses { display: grid; gap: 0.85rem; }
        .decision-card { display: grid; grid-template-columns: 3.4rem 1fr; gap: 1rem; padding: 1.3rem; border: 1px solid rgba(0,90,140,0.12); border-radius: 19px; background: rgba(255,255,255,0.78); box-shadow: 0 10px 28px rgba(15,50,68,0.055); }
        .decision-number { display: grid; place-items: center; align-self: start; width: 3.4rem; height: 3.4rem; border-radius: 15px; background: linear-gradient(145deg, var(--aba-blue), var(--aba-slate)); color: #fff; font-size: 0.73rem; font-weight: 800; letter-spacing: 0.08em; box-shadow: 0 8px 20px rgba(0,90,140,0.18); }
        .decision-card h3 { margin: 0.1rem 0 0.45rem; color: #15394c; font-size: 1.08rem; font-weight: 800; }
        .decision-card p { margin: 0; color: #61717a; font-family: var(--aba-serif); line-height: 1.55; }
        .access-panel { position: relative; display: grid; grid-template-columns: auto 1fr; gap: 1.35rem; align-items: center; margin-top: 2rem; padding: clamp(1.5rem, 4vw, 2.25rem); overflow: hidden; border-radius: 24px; background: linear-gradient(130deg, var(--aba-blue), #174d6d 68%, var(--aba-slate)); color: #fff; box-shadow: 0 24px 55px rgba(0,90,140,0.2); }
        .access-panel::after { content: ''; position: absolute; right: -3rem; bottom: -5rem; width: 15rem; height: 15rem; border: 1px solid rgba(255,255,255,0.15); border-radius: 50%; box-shadow: 0 0 0 2.5rem rgba(255,255,255,0.025); }
        .access-marker { position: relative; z-index: 1; display: grid; place-items: center; width: 5.5rem; height: 5.5rem; border: 1px solid rgba(240,196,91,0.55); border-radius: 50%; color: #f1c65f; font-size: 0.7rem; font-weight: 800; letter-spacing: 0.12em; text-transform: uppercase; }
        .access-copy { position: relative; z-index: 1; }
        .access-copy h3 { margin: 0; font-size: clamp(1.45rem, 3vw, 2.25rem); font-weight: 700; letter-spacing: -0.025em; }
        .access-copy p { max-width: 850px; margin: 0.65rem 0 0; color: rgba(240,248,252,0.76); font-family: var(--aba-serif); font-size: 1.03rem; line-height: 1.62; }
        .mentor-band { padding: 0 0 7rem; background: #f5f8fa; color: var(--ink); }
        .mentor-panel { display: grid; grid-template-columns: 1fr auto; gap: 2rem; align-items: center; padding: clamp(2rem, 5vw, 3.4rem); border: 1px solid rgba(0,90,140,0.14); border-radius: 28px; background: linear-gradient(135deg, #fff, #e9f3f7); box-shadow: 0 20px 50px rgba(12,54,76,0.09); }
        .mentor-panel h2 { margin: 0; color: var(--aba-blue); font-size: clamp(1.8rem, 4vw, 3rem); font-weight: 700; letter-spacing: -0.035em; }
        .mentor-panel p { max-width: 760px; margin: 0.8rem 0 0; color: #5f6d76; font-family: var(--aba-serif); font-size: 1.06rem; line-height: 1.65; }
        .footer { padding: 2rem 0; border-top: 1px solid rgba(255,255,255,0.1); background: #02101a; color: rgba(255,255,255,0.64); }
        @media (max-width: 991.98px) {
            .navbar .container { align-items: flex-start; gap: 1rem; }
            .nav-actions { max-width: 100%; }
            .hero { min-height: auto; }
            .hero-layout, .perspective { grid-template-columns: 1fr; }
            .decision-layout { grid-template-columns: 1fr; }
            .decision-intro { position: static; }
            .signal-panel { min-height: auto; }
            .insight-grid, .journey-grid { grid-template-columns: repeat(2, minmax(0,1fr)); }
        ...
        @media (max-width: 575.98px) {
            ...
            .nav-button { padding: 0.42rem 0.68rem; font-size: 0.78rem; }
            .hero { padding-top: 4rem; }
            .hero h1 { font-size: clamp(2.8rem, 15vw, 4.3rem); }
            .signal-panel { padding: 0.75rem; }
            .signal-frame { min-height: 450px; padding: 0.95rem; }
            .signal-stat { grid-template-columns: 1fr; gap: 0.7rem; }
            .signal-stat p { max-width: none; }
            .signal-row { grid-template-columns: 100px minmax(70px, 1fr) 1.5rem; gap: 0.45rem; }
            .signal-footer { grid-template-columns: 1fr; }
            .insight-grid, .journey-grid, .transfer-list { grid-template-columns: 1fr; }
            .decision-card { grid-template-columns: 2.8rem 1fr; padding: 1.05rem; }
            .decision-number { width: 2.8rem; height: 2.8rem; }
            .access-panel { grid-template-columns: 1fr; }
            .access-marker { width: 4.5rem; height: 4.5rem; }
            .mentor-panel { grid-template-columns: 1fr; }
        }
        @media (prefers-reduced-motion: reduce) { html { scroll-behavior: auto; } .signal-live::before { animation: none; } * { transition-duration: 0.01ms !important; } }
    </style>
</head>
<body>
    ...
    <nav class="navbar navbar-dark">
        <div class="container">
            <a class="navbar-brand" href="/"><img class="brand-logo" src="/banking-future-logo.svg" alt="" width="42" height="42"><span class="brand-copy"><span class="brand-title">Banking on the <span class="brand-highlight">Future</span></span><span class="brand-attribution">American Bankers Association</span></span></a>
            <div class="nav-actions" aria-label="Primary navigation">
                <a class="nav-button" href="/positions">Positions</a>
                <a class="nav-button nav-primary" href="/match">Career Match</a>
                <a class="nav-button" href="/resume">Resume Review</a>
                <a class="nav-button" href="/favorites">Favorites (<span data-favorite-count>0</span>)</a>
            </div>
        </div>
    </nav>
    ...
    <main>
        <section class="hero">
            <div class="container hero-layout">
                <div class="hero-copy reveal reveal-left">
                    <div class="eyebrow">An ABA career exploration platform</div>
                    <h1>The future of banking <span>has room for you.</span></h1>
                    <p class="hero-lead">Banking is a modern industry powered by technology, trust, teamwork, and human judgment. You do not need one specific major or one perfect starting point—just a place to begin exploring.</p>
                    <div class="hero-actions">
                        <a class="action action-primary" href="/positions">Explore positions <span aria-hidden="true">&rarr;</span></a>
                        <a class="action action-secondary" href="/match">Find my career match</a>
                    </div>
                    <div class="proof-strip" aria-label="Website features">
                        <span>150+ career profiles</span>
                        <span>Personalized matching</span>
                        <span>Resume support</span>
                        <span>ABA talent access</span>
                    </div>
                </div>
                <div class="signal-panel reveal reveal-right reveal-delay-1" aria-label="Banking career opportunity dashboard">
                    <div class="signal-frame">
                        <div class="signal-header">
                            <div><span class="signal-kicker">ABA career intelligence</span><span class="signal-heading">Opportunity map</span></div>
                            <span class="signal-live">Explore</span>
                        </div>
                        <div class="signal-stat">
                            <div class="signal-count">150+</div>
                            <p>curated career profiles across modern banking</p>
                        </div>
                        <div class="signal-spectrum" aria-label="Career ecosystems">
                            <div class="signal-row"><span>Client &amp; Business</span><div class="signal-track"><i style="--fill: 92%"></i></div><b>01</b></div>
                            <div class="signal-row"><span>Markets &amp; Wealth</span><div class="signal-track"><i style="--fill: 76%"></i></div><b>02</b></div>
                            <div class="signal-row"><span>Risk &amp; Controls</span><div class="signal-track"><i style="--fill: 84%"></i></div><b>03</b></div>
                            <div class="signal-row"><span>Tech &amp; Operations</span><div class="signal-track"><i style="--fill: 88%"></i></div><b>04</b></div>
                        </div>
                        <div class="signal-footer">
                            <div class="signal-metric"><strong>Many majors</strong><span>Transferable strengths welcome</span></div>
                            <div class="signal-metric"><strong>Entry points</strong><span>Student to experienced</span></div>
                            <div class="signal-metric"><strong>One platform</strong><span>Explore, match, prepare</span></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <section class="section section-light">
            <div class="container position-relative">
                <div class="section-heading reveal">
                    <div class="section-label">Banking, reconsidered</div>
                    <h2>More paths. More strengths. More impact than most students expect.</h2>
                    <p>Insights from ABA leaders point to an industry that is far broader than the traditional image of banking. Modern banks need people who can build, communicate, analyze, protect, organize, and lead.</p>
                </div>
                <div class="insight-grid">
                    <article class="insight-card reveal" data-index="01"><div class="insight-icon">AI</div><h3>Technology is central</h3><p>Banks are adopting AI, strengthening cybersecurity, improving digital products, and redesigning how customers move and manage money.</p></article>
                    <article class="insight-card reveal reveal-delay-1" data-index="02"><div class="insight-icon">+</div><h3>It is not only about math</h3><p>Communication, problem-solving, relationship building, creativity, ethics, and customer understanding are valuable across the organization.</p></article>
                    <article class="insight-card reveal reveal-delay-2" data-index="03"><div class="insight-icon">360</div><h3>Banking is a team sport</h3><p>Customer-facing and behind-the-scenes teams work together. Strong careers grow through collaboration across products, operations, risk, and technology.</p></article>
                    <article class="insight-card reveal reveal-delay-3" data-index="04"><div class="insight-icon">UP</div><h3>One door opens many</h3><p>You may enter through one function and discover an entirely different direction as you build financial knowledge, leadership, and transferable skills.</p></article>
                </div>
            </div>
        </section>
        ...
        <section class="section section-dark" id="start">
            <div class="container">
                <div class="section-heading reveal">
                    <div class="section-label">Your path through the platform</div>
                    <h2>We'll meet you where you are&mdash;and help you move forward with confidence.</h2>
                    <p>This experience is designed to turn curiosity into action—whether you already know the banking industry or are seeing its possibilities for the first time.</p>
                </div>
                <div class="journey-grid">
                    <a class="journey-card reveal" href="/positions"><h3>Explore positions</h3><p>Browse clearly organized career families and learn what roles do, what they pay, and which skills they value.</p><span class="journey-link">Open the position library &rarr;</span></a>
                    <a class="journey-card reveal reveal-delay-1" href="/match"><h3>Discover your match</h3><p>Answer a conversational quiz about your background, strengths, interests, and preferred ways of working.</p><span class="journey-link">Take the career quiz &rarr;</span></a>
                    <a class="journey-card reveal reveal-delay-2" href="/resume"><h3>Prepare with purpose</h3><p>Get resume feedback and role-focused interview questions so your experience tells a stronger, more relevant story.</p><span class="journey-link">Strengthen your resume &rarr;</span></a>
                    <a class="journey-card reveal reveal-delay-3" href="/resume#aba-talent-title"><h3>Enter the ABA network</h3><p>Submit your resume to ABA's talent pool for consideration by participating member banks and select mentorship opportunities.</p><span class="journey-link">Join the talent pool &rarr;</span></a>
                </div>
            </div>
        </section>
        ...
        <section class="section section-light">
            <div class="container perspective">
                <div class="perspective-mark reveal reveal-left">
                    <div class="perspective-copy"><strong>A different background can be an advantage</strong><span>Your experience may already speak banking.</span></div>
                </div>
                <div class="perspective-text reveal reveal-right reveal-delay-1">
                    <h2>You do not have to begin in finance to belong here.</h2>
                    <p>Banking careers are often shaped by unexpected connections and transferable strengths. Experience in public service can lead toward fraud prevention. Technology skills can support secure digital banking. Communication, psychology, law, design, and community work can all translate into meaningful roles.</p>
                    <p>What matters is learning how your strengths solve real problems for customers, teams, businesses, and communities.</p>
                    <div class="transfer-list" aria-label="Transferable strengths">
                        <span>Customer understanding</span><span>Clear communication</span><span>Analytical thinking</span><span>Team leadership</span><span>Technology fluency</span><span>Ethical judgment</span>
                    </div>
                </div>
            </div>
        </section>

        <section class="section decision-section" aria-labelledby="career-decision-title">
            <div class="container position-relative">
                <div class="decision-layout">
                    <div class="decision-intro reveal reveal-left">
                        <div class="section-label">A practical career lens</div>
                        <h2 id="career-decision-title">Choose the work, not only the industry label.</h2>
                        <p>Banking contains dramatically different jobs. The day-to-day experience of two roles inside one bank may differ more than the experience of the same specialty inside and outside banking.</p>
                        <div class="role-signal"><span>Better first question</span><strong>What kind of problems, pace, people, and responsibilities fit me?</strong></div>
                    </div>
                    <div class="decision-lenses">
                        <article class="decision-card reveal reveal-right"><div class="decision-number">01</div><div><h3>Start with the role</h3><p>Compare the actual work, skills, team, and measures of success. A career title and function often tell you more than the industry label alone.</p></div></article>
                        <article class="decision-card reveal reveal-right reveal-delay-1"><div class="decision-number">02</div><div><h3>Expect a fast learning curve</h3><p>Markets, technology, regulation, and customer needs keep changing. Many banking environments reward curiosity and growth, but they can also bring firm deadlines and pressure.</p></div></article>
                        <article class="decision-card reveal reveal-right reveal-delay-2"><div class="decision-number">03</div><div><h3>Learn the business lines</h3><p>Retail, commercial, investment, community, and other bank models serve different customers. Understanding the business line gives every role more context.</p></div></article>
                        <article class="decision-card reveal reveal-right reveal-delay-3"><div class="decision-number">04</div><div><h3>Test the fit through experience</h3><p>An internship, job shadow, project, or informational conversation can reveal the pace and culture in ways that a job description cannot.</p></div></article>
                    </div>
                </div>
                <div class="access-panel reveal">
                    <div class="access-marker">Access</div>
                    <div class="access-copy"><h3>Banking can influence who gets an economic opportunity.</h3><p>Credit helps people, farms, and businesses invest in what comes next. Work that responsibly expands access for historically underserved communities can make banking part of addressing long-standing economic gaps.</p></div>
                </div>
            </div>
        </section>

        <section class="mentor-band">
            <div class="container">
                <div class="mentor-panel reveal">
                    <div><h2>Your voice can create momentum.</h2><p>Ask thoughtful questions. Request opportunities to shadow. Learn from feedback, build relationships, and share fresh ideas. Banking continues to evolve, and emerging professionals can help shape what comes next.</p></div>
                    <a class="action action-primary" href="/match">Begin with your match &rarr;</a>
                </div>
            </div>
        </section>
    </main>
    ...
    <footer class="footer">
        <div class="container d-flex flex-wrap justify-content-between gap-2"><span>Banking on the Future &copy; 2026</span><span>American Bankers Association</span></div>
    </footer>
    <script>
        (() => {
            ...
            const updateScrollEffects = () => {
                ...
                document.documentElement.style.setProperty('--grid-offset', `${-progress * 54}px`);
                ticking = false;
            };
...
<head>
    ...
    <style>
        ...
        .career-details { margin-top: 2.25rem; padding-top: 1.75rem; border-top: 1px solid #dce5ea; }
        .career-path-box { position: relative; overflow: hidden; padding: clamp(1.4rem, 4vw, 2.2rem); border: 1px solid rgba(93,164,199,0.25); border-radius: 24px; background: linear-gradient(145deg, #063654 0%, #005a8c 55%, #4e7c9c 115%); color: #ffffff; box-shadow: 0 22px 55px rgba(0,90,140,0.2), inset 0 1px rgba(255,255,255,0.1); }
        .career-path-box::before { content: ''; position: absolute; right: -7rem; top: -9rem; width: 24rem; height: 24rem; border: 1px solid rgba(255,255,255,0.13); border-radius: 50%; box-shadow: 0 0 0 4rem rgba(255,255,255,0.025), 0 0 0 8rem rgba(255,255,255,0.018); }
        .career-path-box::after { content: ''; position: absolute; inset: 0; pointer-events: none; background-image: linear-gradient(rgba(150,211,238,0.045) 1px, transparent 1px), linear-gradient(90deg, rgba(150,211,238,0.045) 1px, transparent 1px); background-size: 34px 34px; mask-image: linear-gradient(120deg, black, transparent 78%); }
        .career-path-header { position: relative; z-index: 2; display: flex; justify-content: space-between; gap: 1.25rem; align-items: flex-start; margin-bottom: 1.65rem; }
        .career-path-eyebrow { display: block; margin-bottom: 0.38rem; color: #f1c65f; font-size: 0.68rem; font-weight: 800; letter-spacing: 0.15em; text-transform: uppercase; }
        .career-path-header h2 { margin: 0; color: #fff; font-size: clamp(1.6rem, 3vw, 2.25rem); font-weight: 700; letter-spacing: -0.035em; }
        .career-path-header p { max-width: 650px; margin: 0.55rem 0 0; color: rgba(239,248,252,0.7); font-family: var(--aba-serif); line-height: 1.55; }
        .career-stage-count { display: inline-flex; flex: 0 0 auto; align-items: center; gap: 0.42rem; padding: 0.48rem 0.72rem; border: 1px solid rgba(240,196,91,0.42); border-radius: 999px; background: rgba(220,159,29,0.1); color: #f1c65f; font-size: 0.7rem; font-weight: 800; letter-spacing: 0.06em; text-transform: uppercase; }
        .career-stage-count::before { content: ''; width: 6px; height: 6px; border-radius: 50%; background: var(--aba-gold); box-shadow: 0 0 9px rgba(220,159,29,0.8); }
        .career-path-track { position: relative; z-index: 2; display: flex; gap: 1rem; overflow-x: auto; padding: 0.25rem 0.1rem 0.9rem; scroll-snap-type: x proximity; scrollbar-width: thin; scrollbar-color: rgba(240,196,91,0.65) rgba(255,255,255,0.08); }
        .career-path-track::-webkit-scrollbar { height: 6px; }
        .career-path-track::-webkit-scrollbar-track { border-radius: 999px; background: rgba(255,255,255,0.08); }
        .career-path-track::-webkit-scrollbar-thumb { border-radius: 999px; background: rgba(240,196,91,0.65); }
        .career-stage { position: relative; flex: 1 0 175px; min-width: 0; scroll-snap-align: start; }
        .career-stage-marker { position: relative; z-index: 2; display: grid; place-items: center; width: 36px; height: 36px; border: 1px solid rgba(240,196,91,0.55); border-radius: 50%; background: #064467; color: #f3cc6d; font-size: 0.7rem; font-weight: 800; box-shadow: 0 0 0 6px rgba(0,90,140,0.86), 0 0 18px rgba(220,159,29,0.18); }
        .career-stage:not(:last-child)::before { content: ''; position: absolute; z-index: 0; left: 36px; top: 18px; width: calc(100% + 1rem - 36px); height: 1px; background: linear-gradient(90deg, rgba(240,196,91,0.7), rgba(131,202,233,0.28)); transform-origin: left; }
        .career-stage-card { min-height: 150px; margin-top: 1rem; padding: 1rem; border: 1px solid rgba(255,255,255,0.13); border-radius: 16px; background: rgba(2,31,47,0.48); box-shadow: inset 0 1px rgba(255,255,255,0.05); backdrop-filter: blur(8px); transition: transform 0.22s ease, border-color 0.22s ease, background 0.22s ease; }
        .career-stage:hover .career-stage-card { transform: translateY(-4px); border-color: rgba(240,196,91,0.48); background: rgba(4,44,65,0.74); }
        .career-stage-phase { display: block; margin-bottom: 0.55rem; color: #86c8e6; font-size: 0.62rem; font-weight: 800; letter-spacing: 0.11em; text-transform: uppercase; }
        .career-stage:last-child .career-stage-phase { color: #f1c65f; }
        .career-stage-card h3 { margin: 0; color: #fff; font-size: 0.96rem; font-weight: 750; line-height: 1.4; overflow-wrap: anywhere; }
        .career-path-note { position: relative; z-index: 2; display: flex; align-items: flex-start; gap: 0.55rem; margin: 0.9rem 0 0; color: rgba(239,248,252,0.58); font-size: 0.72rem; line-height: 1.45; }
        .career-path-note::before { content: 'i'; display: grid; flex: 0 0 18px; place-items: center; width: 18px; height: 18px; border: 1px solid rgba(255,255,255,0.24); border-radius: 50%; color: #b9dceb; font-size: 0.63rem; font-weight: 800; }
        .career-path-enhanced .career-stage { opacity: 0; transform: translateY(22px); transition: opacity 0.55s ease var(--stage-delay), transform 0.55s cubic-bezier(0.22,1,0.36,1) var(--stage-delay); }
        .career-path-enhanced .career-stage:not(:last-child)::before { transform: scaleX(0); transition: transform 0.7s ease calc(var(--stage-delay) + 180ms); }
        .career-path-enhanced.is-visible .career-stage { opacity: 1; transform: translateY(0); }
        .career-path-enhanced.is-visible .career-stage:not(:last-child)::before { transform: scaleX(1); }
        ...
        .career-details { margin-top: 1.5rem; padding-top: 1.5rem; border-top-color: #dfe7ec; }
        .career-path-box { padding: clamp(1.2rem, 3vw, 1.6rem); border-color: #d7e4eb; border-radius: 16px; background: linear-gradient(145deg, #f9fbfc 0%, #edf5f8 100%); color: var(--aba-gray); box-shadow: 0 10px 28px rgba(22,63,84,0.08); }
        .career-path-box::before { right: -9rem; top: -11rem; border-color: rgba(0,90,140,0.08); box-shadow: 0 0 0 4rem rgba(0,90,140,0.018), 0 0 0 8rem rgba(0,90,140,0.012); }
        .career-path-box::after { background-image: linear-gradient(rgba(0,90,140,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(0,90,140,0.025) 1px, transparent 1px); mask-image: linear-gradient(120deg, black, transparent 70%); }
        .career-path-header { margin-bottom: 1.3rem; }
        .career-path-eyebrow { color: var(--aba-slate); }
        .career-path-header h2 { color: var(--aba-blue); font-size: clamp(1.45rem, 3vw, 1.9rem); }
        .career-path-header p { color: #667984; }
        .career-stage-count { border-color: rgba(220,159,29,0.42); background: #fff9e9; color: #8a610c; }
        .career-path-track { gap: 0.85rem; padding-bottom: 0.65rem; scrollbar-color: rgba(78,124,156,0.6) rgba(0,90,140,0.07); }
        .career-path-track::-webkit-scrollbar-track { background: rgba(0,90,140,0.07); }
        .career-path-track::-webkit-scrollbar-thumb { background: rgba(78,124,156,0.6); }
        .career-stage { flex-basis: 175px; }
        .career-stage-marker { border-color: rgba(220,159,29,0.7); background: var(--aba-blue); color: #fff; box-shadow: 0 0 0 5px #f2f8fa, 0 5px 14px rgba(0,90,140,0.18); }
        .career-stage:not(:last-child)::before { background: linear-gradient(90deg, rgba(220,159,29,0.7), rgba(78,124,156,0.28)); }
        .career-stage-card { min-height: 135px; margin-top: 0.85rem; padding: 0.9rem; border-color: #d9e5eb; border-radius: 13px; background: rgba(255,255,255,0.9); box-shadow: 0 6px 18px rgba(25,63,81,0.055); backdrop-filter: none; }
        .career-stage:first-child .career-stage-card { border-top: 3px solid var(--aba-blue); }
        .career-stage:last-child .career-stage-card { border-top: 3px solid var(--aba-gold); }
        .career-stage:hover .career-stage-card { border-color: #a8c7d7; background: #fff; box-shadow: 0 12px 24px rgba(19,64,87,0.1); }
        .career-stage-phase { color: var(--aba-slate); }
        .career-stage:last-child .career-stage-phase { color: #9a6b0d; }
        .career-stage-card h3 { color: #284454; font-size: 0.9rem; }
        .career-path-note { margin-top: 0.65rem; padding-top: 0.75rem; border-top: 1px solid #dce7ec; color: #72818a; }
        .career-path-note::before { border-color: #abc3cf; color: var(--aba-slate); }
        .lead, .entry-snapshot p, .disclosure-body p, .example-card li, .interview-list li { font-family: var(--aba-serif); }
        @media (max-width: 767.98px) {
            .fact-grid { grid-template-columns: 1fr; }
            .career-path-header { display: block; }
            .career-stage-count { margin-top: 0.85rem; }
            .career-stage { flex-basis: min(78vw, 230px); }
        }
        @media (prefers-reduced-motion: reduce) {
            .career-path-enhanced .career-stage { opacity: 1; transform: none; transition: none; }
            .career-path-enhanced .career-stage:not(:last-child)::before { transform: scaleX(1); transition: none; }
        }
    </style>
</head>
<body>
    ...
    <div class="container mt-4 flex-grow-1">
        ...
        <div class="card detail-card p-4">
            ...
            {% if career_stages %}
            <section class="career-details" data-career-path aria-labelledby="career-path-title">
                <div class="career-path-box">
                    <div class="career-path-header">
                        <div>
                            <span class="career-path-eyebrow">Progression map</span>
                            <h2 id="career-path-title">Career Path</h2>
                            <p>See how responsibility and influence can grow over time in this field.</p>
                        </div>
                        <span class="career-stage-count">{{ career_stages|length }} stages</span>
                    </div>
                    <div class="career-path-track" role="list" tabindex="0" aria-label="Career progression stages">
                        {% for stage in career_stages %}
                        <article class="career-stage" role="listitem" style="--stage-delay: {{ loop.index0 * 90 }}ms">
                            <div class="career-stage-marker">{{ loop.index }}</div>
                            <div class="career-stage-card">
                                <span class="career-stage-phase">{% if loop.first %}Starting point{% elif loop.last %}Leadership horizon{% else %}Growth stage{% endif %}</span>
                                <h3>{{ stage }}</h3>
                            </div>
                        </article>
                        {% endfor %}
                    </div>
                    <p class="career-path-note">Career paths are illustrative. Timing and titles vary by bank, team, performance, and individual goals.</p>
                </div>
            </section>
            {% endif %}
        ...
        <div class="mt-4">
            ...
            <a href="/positions" class="btn btn-outline-dark">Back to Positions</a>
        </div>
    </div>
...
<body>
    ...
    <script>
        (() => {
            const careerPath = document.querySelector('[data-career-path]');
            if (!careerPath || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
            careerPath.classList.add('career-path-enhanced');
            const observer = new IntersectionObserver(([entry]) => {
                careerPath.classList.toggle('is-visible', entry.isIntersecting);
            }, { threshold: 0.22, rootMargin: '0px 0px -8% 0px' });
            observer.observe(careerPath);
        })();
    </script>
    {{ favorite_script | safe }}
</body>
...
@app.route('/positions')
def positions():
    """Position library showing all banking roles by career family."""
    category_details = [
        {
...
@app.route('/')
def index():
    """Introduce the platform and guide visitors to its career tools."""
    return render_template_string(
        index_html,
...
def job_detail(job_id):
    ...
    career_stages = [
        stage.strip()
        for stage in job.get("career_path", "").split("→")
        if stage.strip()
    ]
